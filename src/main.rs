...
fn main() {
    ...
    if args.len() < 2 {
        println!("1 argument needs to be passed in, the target hostname ip or --version,
    ...
    for i in start_port..end_port {
        ...
        let thread = thread::spawn(move || {
            ...
            let hostname = inp.clone();
            let server_port_fmt = format!("{hostname}:{lock_acc}");

            println!("Checking {}...", server_port_fmt);

            if let Ok(_e) = TcpStream::connect(&server_port_fmt) {
                println!("Port {} is open!", lock_acc);
                println!("Addr: {}", server_port_fmt);
                let mut access = port_clone.lock().unwrap();
                access.push(i);
            } else if let Err(e) = TcpStream::connect(&server_port_fmt) {
